...
<template>
  ...
  >
    ...
    >
      <!-- Avatar do Mentor -->
      <div v-if="tipo === 'mentor'" class="flex items-start gap-3 mb-2">
        ...
        <div class="flex-1">
          <p class="text-xs font-semibold text-muted-foreground">Mentor IA</p>
        </div>
      </div>
      ...
      <div 
        v-if="tipo === 'mentor'"
        class="text-sm leading-relaxed prose prose-sm dark:prose-invert max-w-none prose-p:my-2 prose-headings:my-2 prose-ul:my-1 prose-li:my-0"
      ...
      <div class="flex items-center justify-between mt-2 gap-2">
        ...
        <button
          v-if="tipo === 'mentor'"
          @click="copiarTexto"
          class="text-xs px-2 py-1 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors inline-flex items-center gap-2"
