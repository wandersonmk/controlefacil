...
<template>
  <div class="bg-card border-t border-border p-2">
    ...
    <div v-if="showBlockedMessage" class="mb-2 p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg">
      <div class="flex items-start gap-2">
        <span class="text-lg">⚠️</span>
        <div class="flex-1">
          <p class="text-sm font-semibold text-red-700 dark:text-red-400">Tokens insuficientes</p>
          <p class="text-xs text-red-600 dark:text-red-500 mt-0.5">Aguarde a renovação mensal ou adquira tokens extras.</p>
        </div>
      </div>
    </div>
    ...
    <div class="flex gap-2 items-start">
      ...
      <div class="flex-1">
        <textarea
          ...
          rows="2"
          class="w-full px-3 py-2 bg-background border border-border rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-primary transition-all text-sm disabled:opacity-50 disabled:cursor-not-allowed"
        ></textarea>
        <p v-if="!showBlockedMessage" class="text-[10px] text-muted-foreground mt-0.5 px-1">
      ...
      <button
        ...
        class="px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-all flex items-center gap-1.5 shadow-sm text-sm h-[44px]"
      >
